...
public:
  CorruptedPages();
  ~CorruptedPages();
  void add_page(const char *table_name, unsigned page_no);
  bool contains(const char *table_name, unsigned page_no);
...
private:
  typedef std::map<std::string, std::set<unsigned> > container;
