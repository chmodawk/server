...
void CorruptedPages::add_page(const char *table_name, unsigned page_no)
{
  ut_a(!pthread_mutex_lock(&m_mutex));
  (void)m_pages[table_name].insert(page_no);
  ut_a(!pthread_mutex_unlock(&m_mutex));
}

bool CorruptedPages::contains(const char *table_name, unsigned page_no)
{
  bool result = false;
  ut_a(!pthread_mutex_lock(&m_mutex));
  container::const_iterator table_it= m_pages.find(table_name);
  if (table_it != m_pages.end())
    result = table_it->second.count(page_no);
  ut_a(!pthread_mutex_unlock(&m_mutex));
  return result;
}

void CorruptedPages::process_ddl(
    ...
    const std::map<std::string, std::string> &renamed_tables)
{
  ...
  for (std::set<std::string>::const_iterator it= new_tables.begin();
       it != new_tables.end(); ++it)
    m_pages.erase(*it);

  for (std::set<std::string>::const_iterator it= dropped_tables.begin();
       it != dropped_tables.end(); ++it)
    m_pages.erase(*it);

  for (std::map<std::string, std::string>::const_iterator it=
           renamed_tables.begin();
       it != renamed_tables.end(); ++it)
  {
    if (m_pages.find(it->first) == m_pages.end())
      continue;
    m_pages[it->second]= m_pages[it->first];
    m_pages.erase(it->first);
  }

  ut_a(!pthread_mutex_unlock(&m_mutex));
}

bool CorruptedPages::print_to_file(const char *filename)
{
  ...
  for (container::const_iterator space_it=
           m_pages.begin();
       space_it != m_pages.end(); ++space_it)
  {
    out << space_it->first << "\n";
    bool first_page_no= true;
    for (container::mapped_type::const_iterator page_it=
             space_it->second.begin();
         page_it != space_it->second.end(); ++page_it)
      if (first_page_no)
      {
        out << *page_it;
        first_page_no= false;
...
void CorruptedPages::read_from_file(const char *file_name)
{
  ...
  while (std::getline(infile, line))
  {
    ...
    else
    {
      ...
      unsigned page_no;
      while ((iss >> page_no))
        add_page(table_name.c_str(), page_no);
      if (!iss.eof())
        die("Corrupted pages file parse error on line number %lu",
            line_number);
    }
  }
}

bool CorruptedPages::empty()
{
  ut_a(!pthread_mutex_lock(&m_mutex));
  bool result= !m_pages.size();
  ut_a(!pthread_mutex_unlock(&m_mutex));
  return result;
}
...
{
  container non_free_pages;
  byte *zero_page=
      static_cast<byte *>(aligned_malloc(srv_page_size, srv_page_size));
  memset(zero_page, 0, srv_page_size);

  ut_a(!pthread_mutex_lock(&m_mutex));
  for (container::const_iterator space_it= m_pages.begin();
       space_it != m_pages.end(); ++space_it)
  {
    const std::string &space_name = space_it->first;
         ...
         page_it != space_it->second.end(); ++page_it)
    {
      ...
      if (!is_free) {
        non_free_pages[space_name].insert(*page_it);
        msg("Error: corrupted page %u of tablespace %s can not be fixed",
            *page_it, space_name.c_str());
      }
      else
      {
        auto err= space->acquire()
                      ? space
                            ->io(IORequestWrite, *page_it * srv_page_size,
                                 srv_page_size, zero_page)
                            .err
                      : DB_ERROR;
        if (err != DB_SUCCESS)
          die("Can't zero out corrupted page %u of tablespace %s",
              *page_it, space_name.c_str());
        msg("Corrupted page %u of tablespace %s was successfuly fixed.",
            *page_it, space_name.c_str());
      }
    }
  }
  m_pages.swap(non_free_pages);
  ut_a(!pthread_mutex_unlock(&m_mutex));
...
void backup_fix_ddl(CorruptedPages &corrupted_pages)
{
	...
	if (opt_log_innodb_page_corruption)
		corrupted_pages.process_ddl(new_tables, dropped_tables, renamed_tables);
