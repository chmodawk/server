...
void CorruptedPages::add_page(const char *table_name, ulint page_no)
{
...
bool CorruptedPages::contains(const char *table_name, ulint page_no)
{
  bool result = false;
  ut_a(!pthread_mutex_lock(&m_mutex));
       ...
       space_it != m_pages.end(); ++space_it)
  {
         ...
         page_it != space_it->second.end(); ++page_it)
    {
      ...
      if (!is_free) {
        ...
        msg("Error: corrupted page %lu of tablespace %s can not be fixed",
      ...
      else
      {
        ...
        if (err != DB_SUCCESS)
          die("Can't zero out corrupted page %lu of tablespace %s",
              *page_it, space_name.c_str());
        msg("Corrupted page %lu of tablespace %s was successfuly fixed.",
            *page_it, space_name.c_str());
      }
    }
  }
